...
class HomePage:
    def __init__(self, parent):
        ...
        # 创建左侧菜单栏
        self.menu_frame = tk.Frame(self.window, width=150, bg="#f0f0f0")
    ...
    def show_marketplace(self):
        self.clear_content()
        tk.Label(self.content_frame, text="商城", font=("Arial", 16)).pack(pady=10)
        ...
        for i, skin in enumerate(skins):
            ...
            skin_frame = tk.Frame(scrollable_frame, relief=tk.RAISED, bd=1)
            skin_frame.grid(row=row, column=col, padx=10, pady=10)
            
            tk.Label(skin_frame, text=skin, width=10, height=3).pack(padx=5, pady=5)
            
            btn_frame = tk.Frame(skin_frame)
            btn_frame.pack(pady=5)
            
            preview_btn = tk.Button(btn_frame, text="预览", width=5,
                                   command=lambda s=skin: self.preview_skin(s))
            preview_btn.pack(side=tk.LEFT, padx=2)
            
            buy_btn = tk.Button(btn_frame, text="购买", width=5,
    ...
    def preview_skin(self, skin_name):
        ...
        # 这里应该加载和显示皮肤动画
        tk.Label(preview_window, text=f"正在预览: {skin_name}", font=("Arial", 14)).pack(pady=20)
        tk.Label(preview_window, text="这里是皮肤动画预览").pack(pady=10)
    ...
    def buy_skin(self, skin_name):
        ...
        if not self.is_logged_in():
            tk.messagebox.showinfo("提示", "请先登录才能购买皮肤")
            return
        ...
        tk.Label(buy_window, text=f"确定购买 {skin_name} 吗?", font=("Arial", 14)).pack(pady=20)
        tk.Label(buy_window, text="价格: 100金币").pack(pady=10)
        
        btn_frame = tk.Frame(buy_window)
        btn_frame.pack(pady=10)
        
        confirm_btn = tk.Button(btn_frame, text="确认购买", 
    ...
    def confirm_purchase(self, skin_name, window):
        window.destroy()
        tk.messagebox.showinfo("购买成功", f"您已成功购买 {skin_name}!")
    
    def show_chatroom(self):
        self.clear_content()
        tk.Label(self.content_frame, text="聊天室", font=("Arial", 16)).pack(pady=10)
        
        # 检查网络连接
        if not self.is_connected():
            tk.Label(self.content_frame, text="无法连接到服务器，请检查网络连接").pack(pady=20)
            return
        
        # 创建聊天室按钮
        create_btn = tk.Button(self.content_frame, text="创建聊天室", 
        ...
        for i, room in enumerate(chatrooms):
            ...
            room_frame = tk.Frame(scrollable_frame, relief=tk.RAISED, bd=1)
            room_frame.grid(row=row, column=col, padx=10, pady=10)
            
            tk.Label(room_frame, text=room, width=15, height=2).pack(padx=5, pady=5)
            
            join_btn = tk.Button(room_frame, text="加入", 
    ...
    def create_chatroom(self):
        ...
        tk.Label(create_window, text="聊天室名称:").pack(pady=10)
        name_entry = tk.Entry(create_window, width=20)
        name_entry.pack(pady=5)
        
        tk.Label(create_window, text="密码(可选):").pack(pady=10)
        password_entry = tk.Entry(create_window, width=20, show="*")
        password_entry.pack(pady=5)
        
        btn_frame = tk.Frame(create_window)
        btn_frame.pack(pady=10)
        
        confirm_btn = tk.Button(btn_frame, text="创建", 
    ...
    def confirm_create_chatroom(self, name, password, window):
        if not name:
            tk.messagebox.showerror("错误", "聊天室名称不能为空")
            return
        
        window.destroy()
        tk.messagebox.showinfo("成功", f"聊天室 '{name}' 创建成功!")
        # 这里应该实际创建聊天室的逻辑
    
    def join_chatroom(self, room_name):
        # 加入聊天室的逻辑
        tk.messagebox.showinfo("加入聊天室", f"已加入 {room_name}")
        # 这里应该实现实际的加入聊天室逻辑
    
    def show_friends(self):
        self.clear_content()
        tk.Label(self.content_frame, text="好友", font=("Arial", 16)).pack(pady=10)
        
        # 检查是否登录
        if not self.is_logged_in():
            tk.Label(self.content_frame, text="请先登录才能查看好友").pack(pady=20)
            return
        ...
        for name, status in friends:
            friend_frame = tk.Frame(self.content_frame)
            friend_frame.pack(fill=tk.X, padx=20, pady=5)
            
            status_color = "green" if status == "在线" else "gray"
            tk.Label(friend_frame, text=name, width=10).pack(side=tk.LEFT)
            tk.Label(friend_frame, text=status, fg=status_color).pack(side=tk.LEFT)
    
    def show_profile(self):
        self.clear_content()
        tk.Label(self.content_frame, text="我的信息", font=("Arial", 16)).pack(pady=10)
        
        # 检查是否登录
        if not self.is_logged_in():
            tk.Label(self.content_frame, text="请先登录才能查看个人信息").pack(pady=20)
            
            login_btn = tk.Button(self.content_frame, text="登录", command=self.show_login)
        ...
        for i, (key, value) in enumerate(user_info.items()):
            tk.Label(info_frame, text=f"{key}: {value}").grid(row=i, column=0, sticky=tk.W, pady=5)
    ...
    def show_login(self):
        ...
        tk.Label(login_window, text="用户名:").pack(pady=10)
        username_entry = tk.Entry(login_window, width=20)
        username_entry.pack(pady=5)
        
        tk.Label(login_window, text="密码:").pack(pady=10)
        ...
        login_btn = tk.Button(btn_frame, text="登录", 
