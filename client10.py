...
class HomePage:
    ...
    def fetch_market_items(self):
        ...
        url = f"{PLATFORM_API}/backstage/market/items"
        headers = {"Authorization": f"Bearer {self.auth.token}"}
        
        try:
            response = requests.get(url, headers=headers)
            if response.status_code == 200:
                self.market_items = response.json().get("items", [])
                # 重新显示商城
                self.show_marketplace()
            else:
                messagebox.showerror("错误", f"获取商城信息失败: {response.status_code}")
        except Exception as e:
            messagebox.showerror("错误", f"获取商城信息失败: {str(e)}")
    ...
    def buy_skin(self, skin_item):
        ...
        buy_window.title(f"购买 - {skin_item.get('name', '未知皮肤')}")
        buy_window.geometry("350x200")
        buy_window.configure(bg='#ecf0f1')
        buy_window.transient(self.window)
        buy_window.grab_set()
        self.center_window(buy_window, 350, 200)
        
        # 使用Frame包装内容，使其居中
        main_frame = ttk.Frame(buy_window, style='Content.TFrame')
        main_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        ttk.Label(main_frame, text=f"确定购买 {skin_item.get('name', '未知皮肤')} 吗?", 
                 font=('Arial', 14), background='#ecf0f1').pack(pady=10)
        ttk.Label(main_frame, text=f"价格: {skin_item.get('price', 0)}金币", background='#ecf0f1').pack(pady=5)
        
        btn_frame = ttk.Frame(main_frame, style='Content.TFrame')
        btn_frame.pack(pady=10)
        
        confirm_btn = ttk.Button(btn_frame, text="确认购买", style='Action.TButton',
                                command=lambda: self.confirm_purchase(skin_item, buy_window))
    ...
    def confirm_purchase(self, skin_item, window):
        """确认购买皮肤"""
        if not self.auth.token:
            messagebox.showerror("错误", "未登录，无法购买")
            window.destroy()
            return
            
        url = f"{PLATFORM_API}/payment/purchase"
        headers = {"Authorization": f"Bearer {self.auth.token}"}
        data = {
            "item_id": skin_item.get("id"),
            "item_type": "skin",
            "price": skin_item.get("price")
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    messagebox.showinfo("购买成功", f"您已成功购买 {skin_item.get('name')}!")
                    # 刷新背包
                    self.fetch_backpack_items()
                else:
                    messagebox.showerror("购买失败", result.get("message", "购买失败"))
            else:
                messagebox.showerror("购买失败", f"购买失败: {response.status_code}")
        except Exception as e:
    ...
    def show_profile(self):
        ...
        if self.backpack_items:
            ...
            for i, item in enumerate(self.backpack_items):
                item_frame = ttk.Frame(backpack_frame, style='Content.TFrame', relief='raised', padding=5)
    ...
    def fetch_backpack_items(self):
        ...
        url = f"{PLATFORM_API}/user/backpack"
        headers = {"Authorization": f"Bearer {self.auth.token}"}
        
        try:
            response = requests.get(url, headers=headers)
            if response.status_code == 200:
                self.backpack_items = response.json().get("items", [])
            else:
                messagebox.showerror("错误", f"获取背包信息失败: {response.status_code}")
        except Exception as e:
            messagebox.showerror("错误", f"获取背包信息失败: {str(e)}")
...
class JiggerClient:
    ...
    async def ws_main(self):
        ...
        try:
            ...
            # 首先发送认证消息
            if self.auth.token and self.auth.openid:
                auth_msg = {
                    "type": "auth",
